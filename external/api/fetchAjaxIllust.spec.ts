...
describe('fetchAjaxIllust', () => {
  describe('エラー時', () => {
    it('エラー文言が含まれる場合は、それを文言としたErrorが送出されること', async () => {
      await expect(fetchAjaxIllust(mockedIllustIds['存在しない・削除された'])).rejects.toThrow('該当作品は削除されたか、存在しない作品IDです。');
    });
    ...
    it('未知のエラーの場合は、フォールバックエラー文言のErrorが送出されること', async () => {
      ...
      await expect(fetchAjaxIllust(illustId)).rejects.toThrow('Something went wrong.');
    });
  });
